...
def weighted_box_fusion(boxes, iou_threshold=0.55):
    """
    Fuse overlapping boxes class-by-class using confidence-weighted coordinates.

    Unlike NMS, this keeps localization evidence from multiple models/TTA views.
    The maximum cluster confidence is retained so a box is not penalized merely
    because only one ensemble member detected it.
    """
    if not boxes:
        return []
    ...
    boxes_by_class = {}
    for box in boxes:
        boxes_by_class.setdefault(box["class"], []).append(box)

    fused_boxes = []
    for cls, class_boxes in boxes_by_class.items():
        clusters = []
        for box in sorted(class_boxes, key=lambda item: item["confidence"], reverse=True):
            best_cluster = None
            best_iou = 0.0
            for cluster in clusters:
                iou = bbox_iou(box["bbox"], cluster["bbox"])
            ...
            if best_cluster is None:
                clusters.append({
                    ...
                    "weighted_sum": [
                        float(value) * float(box["confidence"]) for value in box["bbox"]
                    ],
                    "weight": float(box["confidence"]),
                })
                continue

            confidence = float(box["confidence"])
            best_cluster["weight"] += confidence
            for index, value in enumerate(box["bbox"]):
                best_cluster["weighted_sum"][index] += float(value) * confidence
            best_cluster["bbox"] = [
                value / best_cluster["weight"] for value in best_cluster["weighted_sum"]
            ]
            best_cluster["confidence"] = max(best_cluster["confidence"], confidence)

        for cluster in clusters:
            fused_boxes.append({
                "class": cluster["class"],
                "confidence": round(cluster["confidence"], 4),
                "bbox": [round(value, 1) for value in cluster["bbox"]],
            })

    return fused_boxes


def postprocess_detections(boxes, iou_threshold=0.5, method="nms"):
    if method == "nms":
        return non_maximum_suppression(boxes, iou_threshold=iou_threshold)
    if method == "wbf":
        return weighted_box_fusion(boxes, iou_threshold=iou_threshold)
    raise ValueError("Post-processing method must be 'nms' or 'wbf'.")
